...
ZAD.4.
    ...
    a) > iris 
    b) > iris.log <- cbind.data.frame(Sepal.Length=log(iris$Sepal.Length), Sepal.Width=log(iris$Sepal.Width), Petal.Length=log(iris$Petal.Length), Petal.Width=log(iris$Petal.Width), Speciesiris$Species)
    c) > iris.log.scale <- cbind.data.frame(Sepal.Length=scale(iris.log$Sepal.Length), Sepal.Width=scale(iris.log$Sepal.Width), Petal.Length=scale(iris.log$Petal.Length), Petal.Width=scale(iris.log$Petal.Width), Species=iris.log$Species)
        ? sd(iris.log.scale$Petal.Length) = 1
        ? mean(iris.log.scale$Petal.Length) ?0


ZAD.5.
    a) > iris.log.scale <- subset(iris.log.scale, select = -c(Species))
lub
> iris.log.scale <- iris.log.scale[,-5]
    ...
    c) > iris.pca
        ...
        ? Sdev3 = 0.3647029 
        ? Sdev4 = 0.1656840
    d) > iris.predict <- predict(iris.pca)
...
> points(iris.predict$PC1[iris.predict$Species=="versicolor"], iris.predict$PC2[iris.predict$Species=="versicolor"], type="p", col="blue")
> points(iris.predict$PC1[iris.predict$Species=="virginica"], iris.predict$PC2[iris.predict$Species=="virginica"], type="p", col="green")
