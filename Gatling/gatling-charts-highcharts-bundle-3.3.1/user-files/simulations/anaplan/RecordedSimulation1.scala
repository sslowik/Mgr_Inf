package anaplan

import scala.concurrent.duration._

import io.gatling.core.Predef._
import io.gatling.http.Predef._
import io.gatling.jdbc.Predef._

class RecordedSimulation1 extends Simulation {

	val httpProtocol = http
		.baseUrl("https://us1a.app.anaplan.com")
		.acceptEncodingHeader("gzip, deflate")
		.acceptLanguageHeader("pl")
		.userAgentHeader("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.193 Safari/537.36")

	val headers_0 = Map(
		"Accept" -> "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9",
		"Sec-Fetch-Dest" -> "document",
		"Sec-Fetch-Mode" -> "navigate",
		"Sec-Fetch-Site" -> "none",
		"Sec-Fetch-User" -> "?1",
		"Upgrade-Insecure-Requests" -> "1",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

	val headers_1 = Map(
		"Accept" -> "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9",
		"Sec-Fetch-Dest" -> "document",
		"Sec-Fetch-Mode" -> "navigate",
		"Sec-Fetch-Site" -> "cross-site",
		"Sec-Fetch-User" -> "?1",
		"Upgrade-Insecure-Requests" -> "1",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

	val headers_2 = Map(
		"Accept" -> "text/css,*/*;q=0.1",
		"Sec-Fetch-Dest" -> "style",
		"Sec-Fetch-Mode" -> "no-cors",
		"Sec-Fetch-Site" -> "same-origin",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

	val headers_4 = Map(
		"Accept" -> "*/*",
		"Sec-Fetch-Dest" -> "script",
		"Sec-Fetch-Mode" -> "no-cors",
		"Sec-Fetch-Site" -> "same-origin",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

	val headers_9 = Map(
		"Accept" -> "image/avif,image/webp,image/apng,image/*,*/*;q=0.8",
		"Sec-Fetch-Dest" -> "image",
		"Sec-Fetch-Mode" -> "no-cors",
		"Sec-Fetch-Site" -> "same-origin",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

	val headers_11 = Map(
		"Accept" -> "*/*",
		"Origin" -> "https://fedauth.pg.com",
		"Sec-Fetch-Dest" -> "font",
		"Sec-Fetch-Mode" -> "cors",
		"Sec-Fetch-Site" -> "same-origin",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

	val headers_13 = Map(
		"Accept" -> "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9",
		"Origin" -> "https://fedauth.pg.com",
		"Sec-Fetch-Dest" -> "document",
		"Sec-Fetch-Mode" -> "navigate",
		"Sec-Fetch-Site" -> "same-origin",
		"Sec-Fetch-User" -> "?1",
		"Upgrade-Insecure-Requests" -> "1",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

	val headers_14 = Map(
		"Accept" -> "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9",
		"Origin" -> "https://fedauth.pg.com",
		"Sec-Fetch-Dest" -> "document",
		"Sec-Fetch-Mode" -> "navigate",
		"Sec-Fetch-Site" -> "cross-site",
		"Upgrade-Insecure-Requests" -> "1",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

	val headers_15 = Map(
		"Accept" -> "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9",
		"Sec-Fetch-Dest" -> "document",
		"Sec-Fetch-Mode" -> "navigate",
		"Sec-Fetch-Site" -> "same-origin",
		"Upgrade-Insecure-Requests" -> "1",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

	val headers_19 = Map(
		"Accept" -> "*/*",
		"Sec-Fetch-Dest" -> "empty",
		"Sec-Fetch-Mode" -> "cors",
		"Sec-Fetch-Site" -> "same-origin",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

	val headers_20 = Map(
		"Accept" -> "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9",
		"Sec-Fetch-Dest" -> "iframe",
		"Sec-Fetch-Mode" -> "navigate",
		"Sec-Fetch-Site" -> "same-origin",
		"Upgrade-Insecure-Requests" -> "1",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

	val headers_21 = Map(
		"Accept" -> "application/json",
		"Sec-Fetch-Dest" -> "empty",
		"Sec-Fetch-Mode" -> "cors",
		"Sec-Fetch-Site" -> "same-origin",
		"X-TracePath" -> "appshell",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

	val headers_22 = Map(
		"Accept" -> "application/json",
		"Sec-Fetch-Dest" -> "empty",
		"Sec-Fetch-Mode" -> "cors",
		"Sec-Fetch-Site" -> "same-origin",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

	val headers_27 = Map(
		"Accept" -> "application/json",
		"Content-Type" -> "application/json",
		"Origin" -> "https://us1a.app.anaplan.com",
		"Sec-Fetch-Dest" -> "empty",
		"Sec-Fetch-Mode" -> "cors",
		"Sec-Fetch-Site" -> "same-origin",
		"X-Client-Session-Id" -> "undefined",
		"X-TracePath" -> "home-web",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

	val headers_28 = Map(
		"Accept" -> "application/json",
		"Content-Type" -> "application/json",
		"Sec-Fetch-Dest" -> "empty",
		"Sec-Fetch-Mode" -> "cors",
		"Sec-Fetch-Site" -> "same-origin",
		"X-Client-Session-Id" -> "0yN3nDzGV",
		"X-TracePath" -> "home-web",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

	val headers_32 = Map(
		"Accept" -> "application/json",
		"Content-Type" -> "application/json",
		"Sec-Fetch-Dest" -> "empty",
		"Sec-Fetch-Mode" -> "cors",
		"Sec-Fetch-Site" -> "same-origin",
		"X-Client-Session-Id" -> "MCb0vsDoN",
		"X-TracePath" -> "home-web",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

	val headers_37 = Map(
		"Accept" -> "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9",
		"Sec-Fetch-Dest" -> "document",
		"Sec-Fetch-Mode" -> "navigate",
		"Sec-Fetch-Site" -> "same-origin",
		"Sec-Fetch-User" -> "?1",
		"Upgrade-Insecure-Requests" -> "1",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

	val headers_46 = Map(
		"Accept" -> "*/*",
		"Content-Type" -> "application/json",
		"Origin" -> "https://us1a.app.anaplan.com",
		"Request-Serial-Number" -> "FE2B3C5805094CC981CC15E513E0D41F-2",
		"Sec-Fetch-Dest" -> "empty",
		"Sec-Fetch-Mode" -> "cors",
		"Sec-Fetch-Site" -> "same-origin",
		"X-Auth-Token-Refresh" -> "no",
		"X-Requested-With" -> "XMLHttpRequest",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

	val headers_47 = Map(
		"Accept" -> "*/*",
		"Content-Type" -> "application/json",
		"Origin" -> "https://us1a.app.anaplan.com",
		"Request-Serial-Number" -> "FE2B3C5805094CC981CC15E513E0D41F-1",
		"Sec-Fetch-Dest" -> "empty",
		"Sec-Fetch-Mode" -> "cors",
		"Sec-Fetch-Site" -> "same-origin",
		"X-Auth-Token-Refresh" -> "default",
		"X-Requested-With" -> "XMLHttpRequest",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

	val headers_48 = Map(
		"Accept" -> "*/*",
		"Content-Type" -> "application/json",
		"Origin" -> "https://us1a.app.anaplan.com",
		"Request-Serial-Number" -> "FE2B3C5805094CC981CC15E513E0D41F-3",
		"Sec-Fetch-Dest" -> "empty",
		"Sec-Fetch-Mode" -> "cors",
		"Sec-Fetch-Site" -> "same-origin",
		"X-Auth-Token-Refresh" -> "default",
		"X-Requested-With" -> "XMLHttpRequest",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

	val headers_51 = Map(
		"Accept" -> "*/*",
		"Content-Type" -> "application/json",
		"Origin" -> "https://us1a.app.anaplan.com",
		"Request-Serial-Number" -> "FE2B3C5805094CC981CC15E513E0D41F-4",
		"Sec-Fetch-Dest" -> "empty",
		"Sec-Fetch-Mode" -> "cors",
		"Sec-Fetch-Site" -> "same-origin",
		"X-Auth-Token-Refresh" -> "default",
		"X-Requested-With" -> "XMLHttpRequest",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

	val headers_52 = Map(
		"Accept" -> "application/json, text/plain, */*",
		"Sec-Fetch-Dest" -> "empty",
		"Sec-Fetch-Mode" -> "cors",
		"Sec-Fetch-Site" -> "same-origin",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

	val headers_53 = Map(
		"Accept" -> "*/*",
		"Content-Type" -> "application/json",
		"Origin" -> "https://us1a.app.anaplan.com",
		"Sec-Fetch-Dest" -> "empty",
		"Sec-Fetch-Mode" -> "cors",
		"Sec-Fetch-Site" -> "same-origin",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

	val headers_56 = Map(
		"Accept" -> "*/*",
		"Content-Type" -> "application/json",
		"Origin" -> "https://us1a.app.anaplan.com",
		"Request-Serial-Number" -> "FE2B3C5805094CC981CC15E513E0D41F-5",
		"Sec-Fetch-Dest" -> "empty",
		"Sec-Fetch-Mode" -> "cors",
		"Sec-Fetch-Site" -> "same-origin",
		"X-Auth-Token-Refresh" -> "default",
		"X-Requested-With" -> "XMLHttpRequest",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

	val headers_57 = Map(
		"Accept" -> "*/*",
		"Content-Type" -> "application/json",
		"Origin" -> "https://us1a.app.anaplan.com",
		"Request-Serial-Number" -> "FE2B3C5805094CC981CC15E513E0D41F-6",
		"Sec-Fetch-Dest" -> "empty",
		"Sec-Fetch-Mode" -> "cors",
		"Sec-Fetch-Site" -> "same-origin",
		"X-Auth-Token-Refresh" -> "default",
		"X-Requested-With" -> "XMLHttpRequest",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

	val headers_59 = Map(
		"Accept" -> "*/*",
		"Content-Type" -> "application/json",
		"Origin" -> "https://us1a.app.anaplan.com",
		"Request-Serial-Number" -> "FE2B3C5805094CC981CC15E513E0D41F-7",
		"Sec-Fetch-Dest" -> "empty",
		"Sec-Fetch-Mode" -> "cors",
		"Sec-Fetch-Site" -> "same-origin",
		"X-Auth-Token-Refresh" -> "default",
		"X-Requested-With" -> "XMLHttpRequest",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

	val headers_61 = Map(
		"Accept" -> "*/*",
		"Content-Type" -> "application/json",
		"Origin" -> "https://us1a.app.anaplan.com",
		"Request-Serial-Number" -> "FE2B3C5805094CC981CC15E513E0D41F-8",
		"Sec-Fetch-Dest" -> "empty",
		"Sec-Fetch-Mode" -> "cors",
		"Sec-Fetch-Site" -> "same-origin",
		"X-Auth-Token-Refresh" -> "default",
		"X-Requested-With" -> "XMLHttpRequest",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

	val headers_62 = Map(
		"Accept" -> "*/*",
		"Content-Type" -> "application/json",
		"Origin" -> "https://us1a.app.anaplan.com",
		"Request-Serial-Number" -> "FE2B3C5805094CC981CC15E513E0D41F-9",
		"Sec-Fetch-Dest" -> "empty",
		"Sec-Fetch-Mode" -> "cors",
		"Sec-Fetch-Site" -> "same-origin",
		"X-Auth-Token-Refresh" -> "default",
		"X-Requested-With" -> "XMLHttpRequest",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

	val headers_66 = Map(
		"Accept-Encoding" -> "gzip, deflate",
		"Pragma" -> "no-cache",
		"Proxy-Connection" -> "keep-alive")

	val headers_67 = Map(
		"Accept" -> "*/*",
		"Content-Type" -> "application/json",
		"Origin" -> "https://us1a.app.anaplan.com",
		"Request-Serial-Number" -> "FE2B3C5805094CC981CC15E513E0D41F-10",
		"Sec-Fetch-Dest" -> "empty",
		"Sec-Fetch-Mode" -> "cors",
		"Sec-Fetch-Site" -> "same-origin",
		"X-Auth-Token-Refresh" -> "default",
		"X-Requested-With" -> "XMLHttpRequest",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

	val headers_68 = Map(
		"Accept" -> "*/*",
		"Content-Type" -> "application/json",
		"Origin" -> "https://us1a.app.anaplan.com",
		"Request-Serial-Number" -> "FE2B3C5805094CC981CC15E513E0D41F-11",
		"Sec-Fetch-Dest" -> "empty",
		"Sec-Fetch-Mode" -> "cors",
		"Sec-Fetch-Site" -> "same-origin",
		"X-Auth-Token-Refresh" -> "default",
		"X-Requested-With" -> "XMLHttpRequest",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

	val headers_70 = Map(
		"Accept" -> "*/*",
		"Content-Type" -> "application/json",
		"Origin" -> "https://us1a.app.anaplan.com",
		"Request-Serial-Number" -> "FE2B3C5805094CC981CC15E513E0D41F-12",
		"Sec-Fetch-Dest" -> "empty",
		"Sec-Fetch-Mode" -> "cors",
		"Sec-Fetch-Site" -> "same-origin",
		"X-Auth-Token-Refresh" -> "default",
		"X-Requested-With" -> "XMLHttpRequest",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

	val headers_71 = Map(
		"Accept" -> "*/*",
		"Content-Type" -> "application/json",
		"Origin" -> "https://us1a.app.anaplan.com",
		"Request-Serial-Number" -> "FE2B3C5805094CC981CC15E513E0D41F-13",
		"Sec-Fetch-Dest" -> "empty",
		"Sec-Fetch-Mode" -> "cors",
		"Sec-Fetch-Site" -> "same-origin",
		"X-Auth-Token-Refresh" -> "default",
		"X-Requested-With" -> "XMLHttpRequest",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

	val headers_73 = Map(
		"Accept" -> "*/*",
		"Content-Type" -> "application/json",
		"Expect-Reply-Within" -> "10; workspaceServerGuid=6AF159BF36EA44DAABE4006A395C5A6E userGuid=8a81b013706ef34601709bc23f617f21",
		"Origin" -> "https://us1a.app.anaplan.com",
		"Sec-Fetch-Dest" -> "empty",
		"Sec-Fetch-Mode" -> "cors",
		"Sec-Fetch-Site" -> "same-origin",
		"X-Requested-With" -> "XMLHttpRequest",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

	val headers_74 = Map(
		"Accept" -> "*/*",
		"Content-Type" -> "application/json",
		"Origin" -> "https://us1a.app.anaplan.com",
		"Request-Serial-Number" -> "FE2B3C5805094CC981CC15E513E0D41F-14",
		"Sec-Fetch-Dest" -> "empty",
		"Sec-Fetch-Mode" -> "cors",
		"Sec-Fetch-Site" -> "same-origin",
		"X-Auth-Token-Refresh" -> "default",
		"X-Requested-With" -> "XMLHttpRequest",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

	val headers_76 = Map(
		"Accept" -> "*/*",
		"Content-Type" -> "application/json",
		"Origin" -> "https://us1a.app.anaplan.com",
		"Request-Serial-Number" -> "FE2B3C5805094CC981CC15E513E0D41F-15",
		"Sec-Fetch-Dest" -> "empty",
		"Sec-Fetch-Mode" -> "cors",
		"Sec-Fetch-Site" -> "same-origin",
		"X-Auth-Token-Refresh" -> "default",
		"X-Requested-With" -> "XMLHttpRequest",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

	val headers_77 = Map(
		"Accept" -> "*/*",
		"Content-Type" -> "application/json",
		"Origin" -> "https://us1a.app.anaplan.com",
		"Request-Serial-Number" -> "FE2B3C5805094CC981CC15E513E0D41F-16",
		"Sec-Fetch-Dest" -> "empty",
		"Sec-Fetch-Mode" -> "cors",
		"Sec-Fetch-Site" -> "same-origin",
		"X-Auth-Token-Refresh" -> "default",
		"X-Requested-With" -> "XMLHttpRequest",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

	val headers_78 = Map(
		"Accept" -> "*/*",
		"Content-Type" -> "application/json",
		"Origin" -> "https://us1a.app.anaplan.com",
		"Request-Serial-Number" -> "FE2B3C5805094CC981CC15E513E0D41F-17",
		"Sec-Fetch-Dest" -> "empty",
		"Sec-Fetch-Mode" -> "cors",
		"Sec-Fetch-Site" -> "same-origin",
		"X-Auth-Token-Refresh" -> "no",
		"X-Requested-With" -> "XMLHttpRequest",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

	val headers_80 = Map(
		"Accept" -> "*/*",
		"Content-Type" -> "application/json",
		"Origin" -> "https://us1a.app.anaplan.com",
		"Request-Serial-Number" -> "FE2B3C5805094CC981CC15E513E0D41F-18",
		"Sec-Fetch-Dest" -> "empty",
		"Sec-Fetch-Mode" -> "cors",
		"Sec-Fetch-Site" -> "same-origin",
		"X-Auth-Token-Refresh" -> "default",
		"X-Requested-With" -> "XMLHttpRequest",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

	val headers_81 = Map(
		"Accept" -> "*/*",
		"Content-Type" -> "application/json",
		"Origin" -> "https://us1a.app.anaplan.com",
		"Request-Serial-Number" -> "FE2B3C5805094CC981CC15E513E0D41F-19",
		"Sec-Fetch-Dest" -> "empty",
		"Sec-Fetch-Mode" -> "cors",
		"Sec-Fetch-Site" -> "same-origin",
		"X-Auth-Token-Refresh" -> "no",
		"X-Requested-With" -> "XMLHttpRequest",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

	val headers_82 = Map(
		"Accept" -> "*/*",
		"Content-Type" -> "application/json",
		"Origin" -> "https://us1a.app.anaplan.com",
		"Request-Serial-Number" -> "FE2B3C5805094CC981CC15E513E0D41F-21",
		"Sec-Fetch-Dest" -> "empty",
		"Sec-Fetch-Mode" -> "cors",
		"Sec-Fetch-Site" -> "same-origin",
		"X-Auth-Token-Refresh" -> "no",
		"X-Requested-With" -> "XMLHttpRequest",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

	val headers_83 = Map(
		"Accept" -> "*/*",
		"Content-Type" -> "application/json",
		"Origin" -> "https://us1a.app.anaplan.com",
		"Request-Serial-Number" -> "FE2B3C5805094CC981CC15E513E0D41F-20",
		"Sec-Fetch-Dest" -> "empty",
		"Sec-Fetch-Mode" -> "cors",
		"Sec-Fetch-Site" -> "same-origin",
		"X-Auth-Token-Refresh" -> "default",
		"X-Requested-With" -> "XMLHttpRequest",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

	val headers_84 = Map(
		"Accept" -> "*/*",
		"Content-Type" -> "application/json",
		"Origin" -> "https://us1a.app.anaplan.com",
		"Request-Serial-Number" -> "FE2B3C5805094CC981CC15E513E0D41F-22",
		"Sec-Fetch-Dest" -> "empty",
		"Sec-Fetch-Mode" -> "cors",
		"Sec-Fetch-Site" -> "same-origin",
		"X-Auth-Token-Refresh" -> "default",
		"X-Requested-With" -> "XMLHttpRequest",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

	val headers_85 = Map(
		"Accept" -> "*/*",
		"Content-Type" -> "application/json",
		"Origin" -> "https://us1a.app.anaplan.com",
		"Request-Serial-Number" -> "FE2B3C5805094CC981CC15E513E0D41F-23",
		"Sec-Fetch-Dest" -> "empty",
		"Sec-Fetch-Mode" -> "cors",
		"Sec-Fetch-Site" -> "same-origin",
		"X-Auth-Token-Refresh" -> "default",
		"X-Requested-With" -> "XMLHttpRequest",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

	val headers_87 = Map(
		"Accept" -> "*/*",
		"Content-Type" -> "application/json",
		"Origin" -> "https://us1a.app.anaplan.com",
		"Request-Serial-Number" -> "FE2B3C5805094CC981CC15E513E0D41F-24",
		"Sec-Fetch-Dest" -> "empty",
		"Sec-Fetch-Mode" -> "cors",
		"Sec-Fetch-Site" -> "same-origin",
		"X-Auth-Token-Refresh" -> "default",
		"X-Requested-With" -> "XMLHttpRequest",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

	val headers_88 = Map(
		"Accept" -> "*/*",
		"Content-Type" -> "application/json",
		"Origin" -> "https://us1a.app.anaplan.com",
		"Request-Serial-Number" -> "FE2B3C5805094CC981CC15E513E0D41F-25",
		"Sec-Fetch-Dest" -> "empty",
		"Sec-Fetch-Mode" -> "cors",
		"Sec-Fetch-Site" -> "same-origin",
		"X-Auth-Token-Refresh" -> "default",
		"X-Requested-With" -> "XMLHttpRequest",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

	val headers_89 = Map(
		"Accept" -> "*/*",
		"Content-Type" -> "application/json",
		"Origin" -> "https://us1a.app.anaplan.com",
		"Request-Serial-Number" -> "FE2B3C5805094CC981CC15E513E0D41F-26",
		"Sec-Fetch-Dest" -> "empty",
		"Sec-Fetch-Mode" -> "cors",
		"Sec-Fetch-Site" -> "same-origin",
		"X-Auth-Token-Refresh" -> "default",
		"X-Requested-With" -> "XMLHttpRequest",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

	val headers_90 = Map(
		"Accept" -> "*/*",
		"Content-Type" -> "application/json",
		"Origin" -> "https://us1a.app.anaplan.com",
		"Request-Serial-Number" -> "FE2B3C5805094CC981CC15E513E0D41F-28",
		"Sec-Fetch-Dest" -> "empty",
		"Sec-Fetch-Mode" -> "cors",
		"Sec-Fetch-Site" -> "same-origin",
		"X-Auth-Token-Refresh" -> "no",
		"X-Requested-With" -> "XMLHttpRequest",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

	val headers_91 = Map(
		"Accept" -> "*/*",
		"Content-Type" -> "application/json",
		"Origin" -> "https://us1a.app.anaplan.com",
		"Request-Serial-Number" -> "FE2B3C5805094CC981CC15E513E0D41F-27",
		"Sec-Fetch-Dest" -> "empty",
		"Sec-Fetch-Mode" -> "cors",
		"Sec-Fetch-Site" -> "same-origin",
		"X-Auth-Token-Refresh" -> "default",
		"X-Requested-With" -> "XMLHttpRequest",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

	val headers_92 = Map(
		"Accept" -> "*/*",
		"Content-Type" -> "application/json",
		"Origin" -> "https://us1a.app.anaplan.com",
		"Request-Serial-Number" -> "FE2B3C5805094CC981CC15E513E0D41F-29",
		"Sec-Fetch-Dest" -> "empty",
		"Sec-Fetch-Mode" -> "cors",
		"Sec-Fetch-Site" -> "same-origin",
		"X-Auth-Token-Refresh" -> "default",
		"X-Requested-With" -> "XMLHttpRequest",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

	val headers_93 = Map(
		"Accept" -> "*/*",
		"Content-Type" -> "application/json",
		"Origin" -> "https://us1a.app.anaplan.com",
		"Request-Serial-Number" -> "FE2B3C5805094CC981CC15E513E0D41F-30",
		"Sec-Fetch-Dest" -> "empty",
		"Sec-Fetch-Mode" -> "cors",
		"Sec-Fetch-Site" -> "same-origin",
		"X-Auth-Token-Refresh" -> "default",
		"X-Requested-With" -> "XMLHttpRequest",
		"sec-ch-ua" -> """"Chromium";v="86", "\"Not\\A;Brand";v="99", "Google Chrome";v="86"""",
		"sec-ch-ua-mobile" -> "?0")

    val uri2 = "https://fedauth.pg.com"
    val uri3 = "https://sdp.anaplan.com/frontdoor"
    val uri4 = "http://www.gstatic.com/generate_204"

	val chain_0 = exec(http("request_0")
			.get(uri3 + "/saml/pgfedsaml")
			.headers(headers_0))
		.pause(8)
		.exec(http("request_1")
			.get(uri2 + "/idp/SSO.saml2?SAMLRequest=jVNLa%2BMwEP4rRnc%2FSdwgYpdsStlCl4bY3UMvRZEmqcCWtBo5m5%2B%2Fsh1TH7YlN6OZb74XXt9f2iY4g0WpVUHSKCEBKK6FVKeCvNaP4Yrcl2tkbWPopnMfag9%2FOkAXeJxCOgwK0llFNUOJVLEWkDpOq82vZ5pFCTVWO811Q4INIljnibZaYdeCrcCeJYfX%2FXNBPpwzSOMYhYmYYqZhKuK6jY9WKye0tnHPFZvTEUT%2FRYIHL0Mq5gblE9xPmZcZmdOAlsLEVfUS9YiMBI%2FachhsFOTIGgQSPD0U5D1l%2BWKxzEWY3i0Pd3mSHTIRLhOe%2BznuGKI8wycCsYMnhY4pV5AsyZIwTcM0q9OEpjldptFitXojwe5q%2FIdUY5zfpXQYl5D%2BrOtduHupahL8nmrxC2QqYWC3t8fPptBJ%2BUXE63h%2BeeTJDL0WDWLIy1fm4DKvPbu5961uDbMSeytwYdxdzdD55W3jle7hOGO42di3a5zy%2FrR%2F7ov8q63oiwHundWWKTTaujGC%2F%2Bopx9lXgZRTevOfo%2FwH")
			.headers(headers_1))
		.exec(http("request_2")
			.get(uri2 + "/assets/css/googlefont.css")
			.headers(headers_2))
		.exec(http("request_3")
			.get(uri2 + "/assets/css/pg.prof.main.css")
			.headers(headers_2))
		.exec(http("request_4")
			.get(uri2 + "/assets/js/ladda.min.js")
			.headers(headers_4))
		.exec(http("request_5")
			.get(uri2 + "/assets/css/tmx.css")
			.headers(headers_2))
		.exec(http("request_6")
			.get(uri2 + "/assets/css/ladda-themeless.min.css")
			.headers(headers_2))
		.exec(http("request_7")
			.get(uri2 + "/assets/js/threatmetrix-1.0.js")
			.headers(headers_4))
		.exec(http("request_8")
			.get(uri2 + "/assets/js/spin.min.js")
			.headers(headers_4))
		.exec(http("request_9")
			.get(uri2 + "/assets/images/pg_logo_u13.png")
			.headers(headers_9))
		.exec(http("request_10")
			.get(uri2 + "/assets/js/jquery-2.2.0.min.js")
			.headers(headers_4))
		.exec(http("request_11")
			.get(uri2 + "/assets/fonts/proxima-nova/mem8YaGs126MiZpBA-UFVZ0b.woff2")
			.headers(headers_11))
		.exec(http("request_12")
			.get(uri2 + "/assets/fonts/proxima-nova/mem5YaGs126MiZpBA-UN7rgOUuhp.woff2")
			.headers(headers_11))
		.pause(27)
		.exec(http("request_13")
			.post(uri2 + "/idp/Qgpc9/resumeSAML20/idp/SSO.ping")
			.headers(headers_13)
			.formParam("pf.username", "tisanaplanuseruat.im")
			.formParam("pf.pass", "Osmiornica2/")
			.formParam("pf.ok", "clicked")
			.formParam("pf.adapterId", "PGEDFormAdapter"))
		.exec(http("request_14")
			.post(uri3 + "/saml/pgfedsaml")
			.headers(headers_14)
			.formParam("SAMLResponse", ""))
		.exec(http("request_15")
			.get(uri3 + "/login?service=https://us1a.app.anaplan.com/launchpad/nonpublic/WorkspaceRedirect.action")
			.headers(headers_15))
		.exec(http("request_16")
			.get("/a/springboard-app-shell/static/css/main.811d2ee0.chunk.css")
			.headers(headers_2))
		.exec(http("request_17")
			.get("/a/springboard-app-shell/static/js/2.5437957b.chunk.js")
			.headers(headers_4))
		.exec(http("request_18")
			.get("/a/springboard-app-shell/static/js/main.a9962fc1.chunk.js")
			.headers(headers_4))
		.pause(131 milliseconds)
		.exec(http("request_19")
			.get("/a/springboard-app-shell/static/media/icon-sprite.994bb85f.svg")
			.headers(headers_19))
		.exec(http("request_20")
			.get("/a/home-home-web")
			.headers(headers_20))
		.exec(http("request_21")
			.get("/a/springboard-platform-gateway-service/customers")
			.headers(headers_21))
		.exec(http("request_22")
			.get("/styx/v1/notif/inapp/paged?size=100")
			.headers(headers_22))
		.exec(http("request_23")
			.get("/gnav/v1/bootstrap")
			.headers(headers_22))
		.exec(http("request_24")
			.get("/a/home-home-web/static/js/main.796ce713.chunk.js")
			.headers(headers_4))
		.exec(http("request_25")
			.get("/a/home-home-web/static/css/main.ecf27aa6.chunk.css")
			.headers(headers_2))
		.exec(http("request_26")
			.get("/a/home-home-web/static/js/2.0dd8dc1d.chunk.js")
			.headers(headers_4))
		.pause(185 milliseconds)
		.exec(http("request_27")
			.post("/a/springboard-platform-gateway-service/featureflags/home")
			.headers(headers_27)
			.body(RawFileBody("anaplan/recordedsimulation1/0027_request.json")))
		.exec(http("request_28")
			.get("/a/springboard-platform-gateway-service/token/validate")
			.headers(headers_28))
		.exec(http("request_29")
			.get("/a/springboard-platform-gateway-service/user-info")
			.headers(headers_28))
		.exec(http("request_30")
			.get("/a/home-home-web/static/media/icon-sprite.994bb85f.svg")
			.headers(headers_19))
		.exec(http("request_31")
			.get("/a/home-home-web/static/media/welcome-banner.7ca97885.png")
			.headers(headers_9))
		.exec(http("request_32")
			.get("/a/springboard-definition-service/pages?sort=lastAccessed&order=desc&limit=8")
			.headers(headers_32))
		.exec(http("request_33")
			.get("/a/springboard-definition-service/pages?isFavorite=true&sort=lastAccessed&order=desc&limit=8")
			.headers(headers_32))
		.exec(http("request_34")
			.get("/a/springboard-platform-gateway-service/models?limit=8&offset=0&search=&sort=-LastVisitedOn,+ModelName&includeCategoryValues=false")
			.headers(headers_32))
		.exec(http("request_35")
			.get("/a/springboard-platform-gateway-service/models/favourites")
			.headers(headers_32))
		.exec(http("request_36")
			.get("/a/springboard-definition-service/apps?limit=8")
			.headers(headers_32))
		.pause(7)
		.exec(http("request_37")
			.get("/a/springboard-platform-gateway-service/workspaces/8a81b09e5c6fc013015c9d5f4fe8548f/models/230B849EA10E4E4792110EA7CEDDB5C3/redirect")
			.headers(headers_37))
		.exec(http("request_38")
			.get("/core3996/anaplan/lib-76cf477a-2728-4176-b2a9-2310a7e8a317/modern/modern.css")
			.headers(headers_2))
		.exec(http("request_39")
			.get("/core3996/anaplan/lib-76cf477a-2728-4176-b2a9-2310a7e8a317/dormouse/main/styles.css")
			.headers(headers_2))
		.exec(http("request_40")
			.get("/core3996/anaplan/lib-76cf477a-2728-4176-b2a9-2310a7e8a317/modern/modern.js")
			.headers(headers_4))
		.exec(http("request_41")
			.get("/core3996/anaplan/lib-76cf477a-2728-4176-b2a9-2310a7e8a317/dojo/dojo.js")
			.headers(headers_4))
		.pause(331 milliseconds)
		.exec(http("request_42")
			.get("/core3996/anaplan/lib-76cf477a-2728-4176-b2a9-2310a7e8a317/dojo/nls/dojo_pl.js")
			.headers(headers_4))
		.exec(http("request_43")
			.get("/core3996/anaplan/lib-76cf477a-2728-4176-b2a9-2310a7e8a317/dgrid/css/dgrid.css")
			.headers(headers_2))
		.exec(http("request_44")
			.get("/core3996/anaplan/lib-76cf477a-2728-4176-b2a9-2310a7e8a317/dojo/resources/blank.gif")
			.headers(headers_9))
		.exec(http("request_45")
			.get("/core3996/anaplan/lib-76cf477a-2728-4176-b2a9-2310a7e8a317/dormouse/images/favicon.ico")
			.headers(headers_9))
		.pause(1)
		.exec(http("request_46")
			.post("/core3996/anaplan/jsonrpc")
			.headers(headers_46)
			.body(RawFileBody("anaplan/recordedsimulation1/0046_request.json")))
		.exec(http("request_47")
			.post("/core3996/anaplan/jsonrpc")
			.headers(headers_47)
			.body(RawFileBody("anaplan/recordedsimulation1/0047_request.json")))
		.pause(365 milliseconds)
		.exec(http("request_48")
			.post("/core3996/anaplan/jsonrpc")
			.headers(headers_48)
			.body(RawFileBody("anaplan/recordedsimulation1/0048_request.json")))
		.exec(http("request_49")
			.get("/core3996/anaplan/lib-76cf477a-2728-4176-b2a9-2310a7e8a317/dormouse/images/loadingAnimation.gif")
			.headers(headers_9))
		.exec(http("request_50")
			.get("/core3996/anaplan/lib-76cf477a-2728-4176-b2a9-2310a7e8a317/dormouse/images/sprite.css.svg")
			.headers(headers_9))
		.exec(http("request_51")
			.post("/core3996/anaplan/jsonrpc")
			.headers(headers_51)
			.body(RawFileBody("anaplan/recordedsimulation1/0051_request.json")))
		.exec(http("request_52")
			.get("/styx/v1/notif/inapp/paged?size=100")
			.headers(headers_52))
		.exec(http("request_53")
			.post("/dashboardconfig/workspace/8a81b09e5c6fc013015c9d5f4fe8548f/model/230B849EA10E4E4792110EA7CEDDB5C3/dashboard/115000000627/view/My%20View")
			.headers(headers_53)
			.body(RawFileBody("anaplan/recordedsimulation1/0053_request.json")))
		.exec(http("request_54")
			.get("/core3996/anaplan/lib-76cf477a-2728-4176-b2a9-2310a7e8a317/modern/sprite.css.svg")
			.headers(headers_9))
		.exec(http("request_55")
			.get("/gnav/v1/bootstrap")
			.headers(headers_52))
		.exec(http("request_56")
			.post("/core3996/anaplan/jsonrpc")
			.headers(headers_56)
			.body(RawFileBody("anaplan/recordedsimulation1/0056_request.json")))
		.pause(16)
		.exec(http("request_57")
			.post("/core3996/anaplan/jsonrpc")
			.headers(headers_57)
			.body(RawFileBody("anaplan/recordedsimulation1/0057_request.json")))
		.exec(http("request_58")
			.post("/dashboardconfig/workspace/8a81b09e5c6fc013015c9d5f4fe8548f/model/230B849EA10E4E4792110EA7CEDDB5C3/dashboard/115000000548/view/My%20View")
			.headers(headers_53)
			.body(RawFileBody("anaplan/recordedsimulation1/0058_request.json")))
		.exec(http("request_59")
			.post("/core3996/anaplan/jsonrpc")
			.headers(headers_59)
			.body(RawFileBody("anaplan/recordedsimulation1/0059_request.json")))
		.pause(4)
		.exec(http("request_60")
			.get("/core3996/anaplan/lib-76cf477a-2728-4176-b2a9-2310a7e8a317/dormouse/images/tooltip.png")
			.headers(headers_9))
		.pause(627 milliseconds)
		.exec(http("request_61")
			.post("/core3996/anaplan/jsonrpc")
			.headers(headers_61)
			.body(RawFileBody("anaplan/recordedsimulation1/0061_request.json")))
		.pause(121 milliseconds)
		.exec(http("request_62")
			.post("/core3996/anaplan/jsonrpc")
			.headers(headers_62)
			.body(RawFileBody("anaplan/recordedsimulation1/0062_request.json")))
		.pause(245 milliseconds)
		.exec(http("request_63")
			.get("/core3996/anaplan/lib-76cf477a-2728-4176-b2a9-2310a7e8a317/dormouse/images/i.gif")
			.headers(headers_9))
		.exec(http("request_64")
			.get("/core3996/anaplan/lib-76cf477a-2728-4176-b2a9-2310a7e8a317/dormouse/images/i_half.gif")
			.headers(headers_9))
		.exec(http("request_65")
			.get("/core3996/anaplan/lib-76cf477a-2728-4176-b2a9-2310a7e8a317/dormouse/images/treeExpand_leaf.gif")
			.headers(headers_9))
		.pause(636 milliseconds)
		.exec(http("request_66")
			.get(uri4)
			.headers(headers_66))
		.pause(9)
		.exec(http("request_67")
			.post("/core3996/anaplan/jsonrpc")
			.headers(headers_67)
			.body(RawFileBody("anaplan/recordedsimulation1/0067_request.json")))
		.pause(15)
		.exec(http("request_68")
			.post("/core3996/anaplan/jsonrpc")
			.headers(headers_68)
			.body(RawFileBody("anaplan/recordedsimulation1/0068_request.json")))
		.pause(3)
		.exec(http("request_69")
			.get("/core3996/anaplan/lib-76cf477a-2728-4176-b2a9-2310a7e8a317/dormouse/images/progressBarAnim.gif")
			.headers(headers_9))
		.exec(http("request_70")
			.post("/core3996/anaplan/jsonrpc")
			.headers(headers_70)
			.body(RawFileBody("anaplan/recordedsimulation1/0070_request.json")))
		.exec(http("request_71")
			.post("/core3996/anaplan/jsonrpc")
			.headers(headers_71)
			.body(RawFileBody("anaplan/recordedsimulation1/0071_request.json")))
		.exec(http("request_72")
			.get("/core3996/anaplan/lib-76cf477a-2728-4176-b2a9-2310a7e8a317/dormouse/images/progressBarFull.png")
			.headers(headers_9))
		.pause(872 milliseconds)
		.exec(http("request_73")
			.post("/core3996/anaplan/jsonrpc")
			.headers(headers_73)
			.body(RawFileBody("anaplan/recordedsimulation1/0073_request.json")))
		.pause(5)
		.exec(http("request_74")
			.post("/core3996/anaplan/jsonrpc")
			.headers(headers_74)
			.body(RawFileBody("anaplan/recordedsimulation1/0074_request.json")))
		.pause(1)
		.exec(http("request_75")
			.get("/core3996/anaplan/lib-76cf477a-2728-4176-b2a9-2310a7e8a317/dormouse/images/grid-styles-clean-label-highlight.png")
			.headers(headers_9))
		.pause(2)
		.exec(http("request_76")
			.post("/core3996/anaplan/jsonrpc")
			.headers(headers_76)
			.body(RawFileBody("anaplan/recordedsimulation1/0076_request.json")))
		.pause(12)
		.exec(http("request_77")
			.post("/core3996/anaplan/jsonrpc")
			.headers(headers_77)
			.body(RawFileBody("anaplan/recordedsimulation1/0077_request.json")))

val chain_1 = exec(http("request_78")
			.post("/core3996/anaplan/jsonrpc")
			.headers(headers_78)
			.body(RawFileBody("anaplan/recordedsimulation1/0078_request.json")))
		.pause(6)
		.exec(http("request_79")
			.get("/core3996/anaplan/lib-76cf477a-2728-4176-b2a9-2310a7e8a317/dormouse/images/calendar.png")
			.headers(headers_9))
		.pause(2)
		.exec(http("request_80")
			.post("/core3996/anaplan/jsonrpc")
			.headers(headers_80)
			.body(RawFileBody("anaplan/recordedsimulation1/0080_request.json")))
		.exec(http("request_81")
			.post("/core3996/anaplan/jsonrpc")
			.headers(headers_81)
			.body(RawFileBody("anaplan/recordedsimulation1/0081_request.json")))
		.pause(7)
		.exec(http("request_82")
			.post("/core3996/anaplan/jsonrpc")
			.headers(headers_82)
			.body(RawFileBody("anaplan/recordedsimulation1/0082_request.json")))
		.exec(http("request_83")
			.post("/core3996/anaplan/jsonrpc")
			.headers(headers_83)
			.body(RawFileBody("anaplan/recordedsimulation1/0083_request.json")))
		.pause(11)
		.exec(http("request_84")
			.post("/core3996/anaplan/jsonrpc")
			.headers(headers_84)
			.body(RawFileBody("anaplan/recordedsimulation1/0084_request.json")))
		.pause(6)
		.exec(http("request_85")
			.post("/core3996/anaplan/jsonrpc")
			.headers(headers_85)
			.body(RawFileBody("anaplan/recordedsimulation1/0085_request.json")))
		.pause(7)
		.exec(http("request_86")
			.post("/dashboardconfig/workspace/8a81b09e5c6fc013015c9d5f4fe8548f/model/230B849EA10E4E4792110EA7CEDDB5C3/dashboard/115000000545/view/My%20View")
			.headers(headers_53)
			.body(RawFileBody("anaplan/recordedsimulation1/0086_request.json")))
		.exec(http("request_87")
			.post("/core3996/anaplan/jsonrpc")
			.headers(headers_87)
			.body(RawFileBody("anaplan/recordedsimulation1/0087_request.json")))
		.pause(6)
		.exec(http("request_88")
			.post("/core3996/anaplan/jsonrpc")
			.headers(headers_88)
			.body(RawFileBody("anaplan/recordedsimulation1/0088_request.json")))
		.pause(261 milliseconds)
		.exec(http("request_89")
			.post("/core3996/anaplan/jsonrpc")
			.headers(headers_89)
			.body(RawFileBody("anaplan/recordedsimulation1/0089_request.json")))
		.pause(25)
		.exec(http("request_90")
			.post("/core3996/anaplan/jsonrpc")
			.headers(headers_90)
			.body(RawFileBody("anaplan/recordedsimulation1/0090_request.json")))
		.exec(http("request_91")
			.post("/core3996/anaplan/jsonrpc")
			.headers(headers_91)
			.body(RawFileBody("anaplan/recordedsimulation1/0091_request.json")))
		.pause(411 milliseconds)
		.exec(http("request_92")
			.post("/core3996/anaplan/jsonrpc")
			.headers(headers_92)
			.body(RawFileBody("anaplan/recordedsimulation1/0092_request.json")))
		.pause(16)
		.exec(http("request_93")
			.post("/core3996/anaplan/jsonrpc")
			.headers(headers_93)
			.body(RawFileBody("anaplan/recordedsimulation1/0093_request.json")))

	val scn = scenario("RecordedSimulation1").exec(
		chain_0, chain_1)

	setUp(scn.inject(atOnceUsers(1))).protocols(httpProtocol)
}